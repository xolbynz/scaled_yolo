...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            f = []  # image files
            for p in path if isinstance(path[0], list) else [path[0]]:
        ...
        # Define labels
        self.label_files = [os.path.join(str(Path(path[1])), os.path.splitext(os.path.basename(x))[0]+'.txt') for x in self.img_files]
